# Kickstarter : Application - plateforme d'échanges
Jonathan Bélisle - Session: 1 - Domaine multimédia

## Phrase choc
-   N’avez-vous jamais rêvé d’apprendre ce dont vous avez besoin immédiatement ? Et si tout ce dont vous aviez besoin, un service, un savoir, un coup de main, se trouvait au même endroit.​




## Analyse des besoins

-   Le publique est large, tout le monde peut utiliser cette application. Mais je dirais les personnes ouvertes à l'apprentissage, a l'entraide et ceux qui encourage notre collectivité plutôt que l'intelligence artificiel.​

-   
    
-   Mon projet vise à résoudre l'accès couteux à des service et le gaspille de compétences. ​ ​

## Explication du concept​

### Éléments importants
-   Un site ou tu peux proposer ou demander un micro-service, conseil, correction, révisions et cela dans tous les domaines possibles que ça soit de la cuisine, des devoirs, du sport. Chaque session dure 15 minutes. Pas d’argents, uniquement échanges de temps.​

### Choix technologique
-   Application mobile (ios et android) mais aussi une plateforme web accessible depuis un navigateur. Protection de données et des information personnel. Donc "cross-plateform" et une technologie éthique et responsable.​

### En quoi le concept répond au attentes du publique?
-   Tout le monde à accès, tout le monde peut donner 15 minutes, c'est facile a utilisé et cela respect tout le monde ceux qui ne veulent rester anonyme. Chaque compétence a un sens et de la valeur. Un apprentissage plus accessible et plus juste. C'est un outil adapté aux rythmes de chaque personne.​

## Moodboard

![moodboard.png](https://i.postimg.cc/k5VGX4yd/moodboard.png)

## Tâches principales

### Préproduction
-   Définition du concept et des fonctionnalités clés​
-   Compréhension des besoins des futurs utilisateurs​
-   Création des parcours et maquettes de l’application​  
-   Choix des technologies et planification du projet​
### Production
-   Développement de l’application mobile et de la plateforme web​
-   Création des profils, des compétences et du système d’échange​
-   Mise en place des outils de communication​
-   Sécurisation des données et des comptes​

### Postproduction
-   Tests avec les premiers utilisateurs​
-   Corrections et améliorations​
-   Optimisation de l’expérience utilisateur​
-   Mise en ligne et lancement officiel​
### Opération
-   Maintenance et mises à jour régulières​
-   Support et animation de la communauté​
-   Évolution des fonctionnalités selon les retours 
-   Développement de la plateforme sur le long terme​

## Conclusion
-   Le projet transforme les compétences et le temps en une monnaie d’échange, rendant l’apprentissage accessible sans barrière financière. Il propose un modèle humain et collaboratif, pas nécessairement comme plateformes traditionnelles.​
    
-   Contrairement aux plateformes basées sur le paiement, notre solution repose sur l’échange équitable, la valorisation de toutes les compétences et la création d’une communauté engagée.​
    
-   Le projet répond à un besoin réel, s’inscrit dans l’économie collaborative et possède un fort potentiel de croissance grâce aux réseau sociaux.​  
    En le soutenant, ils feraient parti d’une nouvelle façon d’apprendre et de partager.​




